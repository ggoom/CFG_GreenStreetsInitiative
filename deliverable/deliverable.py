...
def sankey_first_layer(request):
     vars homezip_input=input(form)
...
#onchange:py.plot(fig, filename='sankey_override')
...
def index(request):
    return HttpResponse(tls.get_embed('https://plot.ly/~Jocelyn-Ting/61'))

urlpatterns = [
    url(r'^deliverable/', index)
]
